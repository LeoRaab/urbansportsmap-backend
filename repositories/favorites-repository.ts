...
class FavoritesRepository extends BaseRepository<IUserDoc> {
    ...
    async createFavorite(userId: string, venueId: string): Promise<{ favorites?: Types.Array<IVenueDoc>, error?: HttpError }> {
    ...
    async deleteFavorite(userId: string, venueId: string): Promise<{ favorites?: Types.Array<IVenueDoc>, error?: HttpError }> {
        const {user, venue, error} = await this.prepareFavorites(userId, venueId);
    ...
    private async prepareFavorites(userId: string, venueId: string): Promise<{ user?: IUserDoc, venue?: IVenueDoc, error?: HttpError }> {
